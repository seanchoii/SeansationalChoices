export const restaurants = [
    // sushi
    {
        name: "Kinjo Sushi & Grill Dalhousie 🍣",
        type: 'sushi',
        address: "5005, 415 Dalhousie Dr NW #415, Calgary, AB T3A 5R8",
        latitude: 51.10503,
        longitude: -114.16220,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "- I used to work here, so I tried pretty much every menu here",
        review2: "- Their food is good, not anything spectacular, but they are good",
        review3: "- Really like their potato cakes and beef tataki"
    },
    
    {
        name: "Kinjo Sushi & Grill North Hill 🍣",
        type: 'sushi',
        address: "1632 14 Ave NW #1668, Calgary, AB T2N 1M7",
        latitude: 51.06638,
        longitude: -114.10081,
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "- Similar to Dalhousie one, their food is not spectacular, but not bad",
        review2: "- I went there when the place just opened, so it was not as organized as the Dalhousie one",
        review3: "- Like their beef tataki and volcano rolls"
    },

    {
        name: "Ari Sushi 🍣",
        type: 'sushi',
        address: "1201 9 Ave SE, Calgary, AB T2G 0S9",
        latitude: 51.04183968891738,  
        longitude: -114.0364355301443,
        rating: "⭐⭐⭐⭐⭐⭐ 6/10",
        review1: "- High quality sushi place",
        review2: "- The price is a bit high. The bill was over $100 for two people",
        review3: "- Their food is good especially Hamachi, however knowing its price, it was a bit disappointing"
    },

    {
        name: "Fusion Sushi - Ranchlands 🍣",
        type: 'sushi',
        address: "1829 Ranchlands Blvd NW #159, Calgary, AB T3G 2A7",
        latitude: 51.12340437258305,
        longitude: -114.19834289677064,
        rating: "⭐⭐⭐ 3/10",
        review1: "- All you can eat sushi place",
        review2: "- Most foods were dry or soggy, Especially sashimi had weird fishy smell",
        review3: "- Their service was slow and unfriendly, I would never come back to this place"
    },

    {
        name: "Sushi & Kitchen 🍣",
        type: 'sushi',
        address: "5620 Signal Hill Centre SW, Calgary, AB T3H 3P8",
        latitude: 51.019344791750214, 
        longitude: -114.16882113725244,
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "- All you can eat sushi place",
        review2: "- A lot of variety on the menu, their sushi rolls are great, however sashimi is quite disappointing",
        review3: "- Their customer service was friendly, but a bit slow at times"
    },

    {
        name: "Takumi Sushi 🍣",
        type: 'sushi',
        address: "150 Crowfoot Crescent NW #311, Calgary, AB T3G 2L5",
        latitude: 51.12727242460731, 
        longitude: -114.1957504531855,
        rating: "⭐⭐⭐⭐⭐⭐ 6/10",
        review1: "- Sushi place containing a variety of sushi menus",
        review2: "- Their sushi rolls are great, but sushi and sashimi were not as good",
        review3: "- The tuna sashimi still contained fishbones, and I almost choked :( not a good experience"
    },

    {
        name: "Kibo Sushi Calgary Coach Hill 🍣",
        type: 'sushi',
        address: "6490 Old Banff Coach Rd SW #108, Calgary, AB T3H 5R8",
        latitude: 51.061124533610204,
        longitude: -114.18092291512714,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "- Sushi place containing a variety of sushi menus",
        review2: "- Their spicy salmon and tuna rolls were delicious",
        review3: "- They usually have great deals on Uber Eats"
    },
    
    {
        name: "Sushi Boat 🍣",
        type: 'sushi',
        address: "806 Crowfoot Crescent NW, Calgary, AB T3G 4S3",
        latitude: 51.12720457704648, 
        longitude: -114.20639412530971,
        rating: "⭐⭐⭐⭐⭐ 5/10",
        review1: "- Each dish passes by your table on a floating Sushi boat, fun experience :)",
        review2: "- Their food was not my cup of tea, did not really like their rolls or sushi",
        review3: "- They do have great deals on TooGoodToGo, which is an app where restaurants sell leftovers for cheap"
    },

    {
        name: "Banzai Sushi & Teriyaki House 🍣",
        type: 'sushi',
        address: "1104 20 Ave NW unit 1, Calgary, AB T2M 3M4",
        latitude: 51.070804014473154, 
        longitude: -114.08514075583035 ,
        rating: "⭐⭐⭐ 3/10",
        review1: "- A sushi place selling sushi, rolls, and poke bowls",
        review2: "- Had their salmon poke bowl, and I did not like it. It had small portions, not enough sauce, and their salmon had fishy smell",
        review3: "- I would personally never eat from here again"
    },

    {
        name: "Pocket Holic 🍣",
        type: 'sushi',
        address: "304 10 St NW, Calgary, AB T2N 1V8",
        latitude:  51.05427362102163, 
        longitude: -114.08570940855583,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "- A delicious sushi place located in the heart of Kensington",
        review2: "- Really enjoyed eating their signature pocket sushis. Highly recommend spicy tuna and sweet chili shrimp",
        review3: "- They usually have great deals on Uber eats"
    },

    {
        name: "Sensei Bar 🍣",
        type: 'sushi',
        address: "1520 14 St SW, Calgary, AB T3C 1C9",
        latitude:  51.038634239418776, 
        longitude: -114.09439600381978 ,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "- Delicious Fusion Japanese Bar located in Hillhurst",
        review2: "- They had the best beef tataki i have ever had. They have great happy hour menu containing bao and drinks",
        review3: "- The only reason this restaurant is not a 10 is because their carbonara ramen was terrible"
    },

    {
        name: "Lonely Mouth Bar 🍣",
        type: 'sushi',
        address: "528 17 Ave SW, Calgary, AB T2S 0B1",
        latitude:  51.03800715594472, 
        longitude: -114.0731023903087 ,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 10/10",
        review1: "- Japanese inspired bar serving an eclectic variety of Modern Japanese cuisines and beverages",
        review2: "- Really like their interior, very cozy and warm. Their happy hour is great containing a lot of food menus for cheap",
        review3: "- The food is amazing. Highly recommend AB Beef Handrolls and Kake udon"
    },

    {
        name: "BLACK PEPPER ASIAN STREET FAIR 🍣",
        type: 'sushi',
        address: "1414 8 St SW #110, Calgary, AB T2R 1J6",
        latitude:  51.03977644659879, 
        longitude: -114.08143673604098 ,
        rating: "⭐⭐ 2/10",
        review1: "- A restaurant containing a variety food from Asian countries",
        review2: "- Their beef tataki had a very weird smell to it (DO NOT EAT)",
        review3: "- Their customer service was quite unfriendly and unprofessional"
    },

    // ramen
    {
        name: "Tokyo Street Market 🥡",
        type: 'ramen',
        address: "922 Centre St NE, Calgary, AB T2E 2P7",
        latitude:  51.06053702174044, 
        longitude: -114.06229681569938,
        rating: "⭐⭐⭐⭐⭐⭐ 6/10",
        review1: "- A restaurant that sells food that you normally find in the streets of Japan",
        review2: "- Their ramen is decent, but the rest of the menu is pretty underwhelming. (Black ramen recommended)",
        review3: "- They are owned by the owner of Kinjo, so there isn't much difference from Kinjo"
    },
    
    {
        name: "Muku Japanese Ramen 🥡",
        type: 'ramen',
        address: "326 14 St NW, Calgary, AB T2N 1Z7",
        latitude:  51.05403619086431,  
        longitude: -114.09448931494634,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "- A restaurant that sells ramen at a cheap price with good portion size",
        review2: "- Their ramen is good. Highly recommend their Tonkotsu Ramen",
        review3: "- You can add more toppings that cost extra, however since the ramen itself is cheap, I recommend you to add more toppings"
    },

    {
        name: "Menyatai Japanese Noodles 🥡",
        type: 'ramen',
        address: "24 12 St NW, Calgary, AB T2N 1Y3",
        latitude:  51.052170189989,   
        longitude: -114.09173165537375,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "- A restaurant that have quality ramen",
        review2: "- My brother used to work here, so I tried a lot of the menu",
        review3: "- Their ramen is good all around, Highly recommend Spicy Tonkotsu Ramen"
    },

    {
        name: "Shiki Menya 🥡",
        type: 'ramen',
        address: "824 1 Ave NE, Calgary, AB T2E 0C3",
        latitude:  51.05336738156965,  
        longitude: -114.04408136685629 ,
        rating: "⭐⭐⭐⭐⭐ 5/10",
        review1: "- A restaurant located in Bridgeland, it is open only on lunch time",
        review2: "- I heard a lot of good things for this restaurant, and I was excited to try this place",
        review3: "- Their ramen is not bad, however it was a bit greasy and could not finish one bowl"
    },

    {
        name: "JINYA Ramen Bar 🥡",
        type: 'ramen',
        address: "1800 4 St SW #180, Calgary, AB T2S 2S5",
        latitude:  51.03689836216317, 
        longitude: -114.0707853919036,  
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "- The ramen place located near 17th street",
        review2: "- This place has good ramen, however they are a bit pricey, recommend Tonkotsu Black Ramen",
        review3: "- If this place was a bit cheaper, it would have been the best ramen place for me"
    },

    {
        name: "Wami Ootoya Japanese Cuisine 🥡",
        type: 'ramen',
        address: "3820 Brentwood Rd NW #130, Calgary, AB T2L 2L5",
        latitude: 51.08654503927365,  
        longitude: -114.12864035775628,  
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "- A Japanese Cuisine that is located near Brentwood Station below the apartment",
        review2: "- Their ramen is decent with great portion size. Recommend their Shoyu Ramen",
        review3: "- Must be nice living above the restaurant :(("
    },

    // vietnamese
    {
        name: "Pho City 🍜",
        type: 'vietnamese',
        address: "207 1 St SE unit 2, Calgary, AB T2G 2G3",
        latitude: 51.050745054917186,  
        longitude: -114.06058506235375,  
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 10/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Noodle & Grill Express 🍜",
        type: 'vietnamese',
        address: "2500 University Dr NW, Calgary, AB T2N 4V8",
        latitude:51.07816514001141, 
        longitude: -114.13149597492182  ,  
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Bake Chef Co. 🍜",
        type: 'vietnamese',
        address: "2500 University Dr NW, Calgary, AB T2N 4V8",
        latitude: 51.07831843020113,  
        longitude: -114.13167124253299,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Pho Anh 🍜",
        type: 'vietnamese',
        address: "400 Crowfoot Crescent NW, Calgary, AB T3G 5H6",
        latitude: 51.12976016928524,   
        longitude: -114.20037396869178,
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Pho Hoan Pasteur (Royal Oak) 🍜",
        type: 'vietnamese',
        address: "8650 112 Ave NW #8108, Calgary, AB T3R 0R5",
        latitude: 51.151841165667875,    
        longitude: -114.2116701070913,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Pho X.O. 🍜",
        type: 'vietnamese',
        address: "6 Crowfoot Cir NW #109, Calgary, AB T3G 2T3",
        latitude: 51.123398361500904,   
        longitude: -114.20310691007455 ,
        rating: "⭐⭐ 2/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Van Son Vietnamese Cuisine 🍜",
        type: 'vietnamese',
        address: "308 16 Ave NW, Calgary, AB T2M 0H6",
        latitude: 51.06721282460241,   
        longitude: -114.0679572435471,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // korean
    {
        name: "Izumi Kitchen & Bar 🍙",
        type: 'korean',
        address: "935 7 Ave SW, Calgary, AB T2P 1A5",
        latitude: 51.04691412052098,  
        longitude:-114.08327257211921 ,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 10/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Jjamppong 🍙",
        type: 'korean',
        address: "5802 Macleod Trail SW #100, Calgary, AB T2H 0J8",
        latitude: 51.00238298000308,   
        longitude:-114.07131895702435,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Jjawang 🍙",
        type: 'korean',
        address: "40 Christie Park View SW #1150, Calgary, AB T3H 6E7",
        latitude: 51.03738447884853, 
        longitude: -114.17674880246204,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Han Corea 🍙",
        type: 'korean',
        address: "7008 Farrell Rd SE, Calgary, AB T2H 0T2",
        latitude: 50.99224673585495, 
        longitude: -114.05998674192944,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Bowls Korean BBQ 🍙",
        type: 'korean',
        address: "9650 Harvest Hills Blvd N #1301, Calgary, AB T3K 0B3",
        latitude:51.14101001764086, 
        longitude: -114.0700279025381,
        rating: "⭐⭐⭐⭐ 4/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Hanbo Korean Noodle 🍙",
        type: 'korean',
        address: "4340 Macleod Trail SW, Calgary, AB T2G 0A4",
        latitude: 51.01469072197202, 
        longitude: -114.06452146013504,
        rating: "⭐⭐⭐⭐⭐ 5/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "San Dong Banjeom 🍙",
        type: 'korean',
        address: "3803 26 Ave SW, Calgary, AB T3E 6V7",
        latitude: 51.029928418239834, 
        longitude: -114.14183101646614,
        rating: "⭐⭐⭐⭐ 4/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Manna Korean Cuisine 🍙",
        type: 'korean',
        address: "3400 14 St NW #104, Calgary, AB T2K 1H9",
        latitude: 51.082585527122255, 
        longitude: -114.09429906506844,
        rating: "⭐⭐⭐ 3/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Jjawang 🍙",
        type: 'korean',
        address: "40 Christie Park View SW #1150, Calgary, AB T3H 6E7",
        latitude: 51.037393437738274, 
        longitude: -114.17674734774502,
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "The Apron 🍙",
        type: 'korean',
        address: "924 7 Ave SW, Calgary, AB T2P 1A4",
        latitude: 51.04725146937079, 
        longitude: -114.08291436651122,
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Namsan Authentic Korean Cuisine 🍙",
        type: 'korean',
        address: "507 10 St SW, Calgary, AB T2P 2B8",
        latitude: 51.04872476009947, 
        longitude: -114.08627057146813,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Korean Village Restaurant 🍙",
        type: 'korean',
        address: "1324 10 Ave SW, Calgary, AB T3C 0J2",
        latitude: 51.044577067577556, 
        longitude: -114.09274262211191,
        rating: "⭐⭐⭐ 3/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Asadal Korean Cuisine 🍙",
        type: 'korean',
        address: "735 12 Ave SW, Calgary, AB T2R 1J7",
        latitude: 51.04169474596787, 
        longitude: -114.07860990001664, 
        rating: "⭐⭐⭐ 3/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Hankki 🍙",
        type: 'korean',
        address: "8521 Broadcast Ave SW, Calgary, AB T3H 4C7",
        latitude:51.06460111699131, 
        longitude: -114.21004891058887, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Sso Yummy 🍙",
        type: 'korean',
        address: "128 2 Ave SE Unit 117, Calgary, AB T2G 5J5",
        latitude: 51.05130037859774, 
        longitude: -114.06114658796074, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Kim's Katsu 🍙",
        type: 'korean',
        address: "314 10 St NW, Calgary, AB T2N 1V8",
        latitude: 51.054517051908995, 
        longitude: -114.08558339542203 , 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // chicken
    {
        name: "Seoul Fried Chicken Calgary 🍗",
        type: 'chicken',
        address: "2100 4 St SW, Calgary, AB T2S 1W7",
        latitude: 51.03402392611848,  
        longitude: -114.07131479167954,
        rating: "⭐ 1/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Chicko Chicken 🍗",
        type: 'chicken',
        address: "9650 Harvest Hills Blvd N #925, Calgary, AB T3K 0B3",
        latitude: 51.14055454945195, 
        longitude: -114.06790509812238,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Yum Yum BBQ Chicken 🍗",
        type: 'chicken',
        address: "999 36 St NE, Calgary, AB T2A 7X6",
        latitude: 51.06088140917355,  
        longitude: -113.98411812091017,
        rating: "⭐⭐⭐⭐ 4/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Flirty Bird 🍗",
        type: 'chicken',
        address: "314 10 St NW, Calgary, AB T2N 1V8",
        latitude:51.05441731972578, 
        longitude:  -114.08565228519853,
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Birdie 🍗",
        type: 'chicken',
        address: "920 Northmount Dr NW Unit 7, Calgary, AB T2L 0A9",
        latitude: 51.08667919055693,   
        longitude: -114.10368407827407,
        rating: "⭐⭐⭐⭐⭐⭐ 6/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // pizza
    {
        name: "Seniore's Pizza Restaurant 🍕",
        type: 'pizza',
        address: "176 Bedford Dr NE #5, Calgary, AB T3K 2M9",
        latitude:51.12701926705517, 
        longitude: -114.05589008018688 ,
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Papa John's Pizza 🍕",
        type: 'pizza',
        address: "18 Arbour Lake Way NW, Calgary, AB T3G 5X4",
        latitude:51.137960712979876, 
        longitude: -114.18553428352234, 
        rating: "⭐⭐⭐⭐⭐ 5/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Varsity Pizza & Subs 🍕",
        type: 'pizza',
        address: "4624 Varsity Dr NW, Calgary, AB T3A 2L9",
        latitude:51.09355593660302, 
        longitude: -114.154422064784, 
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // hamburger
    {
        name: "Jam's diner 🍔",
        type: 'hamburger',
        address: "1209 9 Ave SE, Calgary, AB T2G 0S9",
        latitude:51.04184862050971, 
        longitude: -114.036216902449, 
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // steak
    {
        name: "Ten Foot Henry 🥩",
        type: 'steak',
        address: "1209 1 St SW, Calgary, AB T2R 0V3",
        latitude:51.04106721106064, 
        longitude: -114.06598670435045, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Joey Eau Claire 🥩",
        type: 'steak',
        address: "208 Barclay Parade SW 200 Eau Claire Market, Calgary, AB T2P 4R5",
        latitude:51.052143178433475, 
        longitude: -114.06930136324868, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Cactus Club Cafe 🥩",
        type: 'steak',
        address: "2612 39 Ave NE, Calgary, AB T1Y 7J9",
        latitude:51.08959625068059, 
        longitude: -114.00071221303848, 
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // chinese
    {
        name: "Bobby Chao's Restaurant 🍚",
        type: 'chinese',
        address: "34 Edgedale Dr NW #18, Calgary, AB T3A 2R4",
        latitude:51.11909201897636, 
        longitude: -114.15679060220815, 
        rating: "⭐⭐⭐⭐⭐⭐ 6/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "U & Me 🍚",
        type: 'chinese',
        address: "233 Centre St S #201, Calgary, AB T2G 2B7",
        latitude:51.05066427649155, 
        longitude: -114.06289944662156, 
        rating: " 0/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Golden Dragon Hot Pot 🍚",
        type: 'chinese',
        address: "30 Brentwood Common NW unit130, Calgary, AB T2L 1K8",
        latitude:51.08732196507249, 
        longitude: -114.1288734304785, 
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // italian
    {
        name: "My Little Italia 🍝",
        type: 'italian',
        address: "2015 4 St SW #5, Calgary, AB T2S 1W6",
        latitude:51.03463456697121, 
        longitude: -114.07177768270242, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "Al's Pizza & Steakhouse 🍝",
        type: 'italian',
        address: "735 Ranchlands Blvd NW, Calgary, AB T3G 2C5",
        latitude:51.115946130692535, 
        longitude: -114.18061037904467, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐⭐ 10/10",
        review1: "",
        review2: "",
        review3: ""
    },

    {
        name: "My Little Italia 🍝",
        type: 'italian',
        address: "314 10 St NW, Calgary, AB T2N 1V8",
        latitude:51.05446477783082,
        longitude:  -114.0855844390485, 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐ 8/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // dessert
    {
        name: "Village Ice Cream 🍦",
        type: 'dessert',
        address: "University District, 4019 University Ave NW, Calgary, AB T3B 6K3",
        latitude: 51.07819654624652, 
        longitude: -114.14506137844538, 
        rating: "⭐⭐⭐⭐⭐⭐⭐ 7/10",
        review1: "",
        review2: "",
        review3: ""
    },

    // other
    {
        name: "Añejo Restaurant 🌮",
        type: 'other',
        address: "University District, 4019 University Ave NW, Calgary, AB T3B 6K3",
        latitude: 51.03370396604457, 
        longitude: -114.07124841551587 , 
        rating: "⭐⭐⭐⭐⭐⭐⭐⭐⭐ 9/10",
        review1: "",
        review2: "",
        review3: ""
    },

]